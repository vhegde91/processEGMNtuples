...
void getEffFakes_MVA_diffRuns(){
  ...
  //  vector<TString> idNames    = {"MVATight","MVAVLooseFO","MVAVLoose"};
  //  vector<TString> refIdNames = {"MVATightNew2","MVAVLooseFONew","MVAVLooseNew"};
  ...
  f[0] = new TFile("mc_eff_2018.root");   col[0] = kGreen+2;
  f[1] = new TFile("mc_fake_2018.root");  col[1] = kGreen+2;
  ...
  for(int i=0;i<idNames.size();i++){
	...
	if(p==0){
	  ...
	  if(logY)  legend[i] = new TLegend(0.25,0.68,0.65,0.78); 
	    ...
	    legend[i] = new TLegend(0.15,0.78,0.65,0.88);
